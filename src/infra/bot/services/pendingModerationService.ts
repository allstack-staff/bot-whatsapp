 ...
 * tratado como se não existisse mais.
 */
export class PendingModerationService {
    async add(groupJid: string, sender: string, text: string): Promise<void> {
        await prisma.pendingModerationMessage.create({ data: { groupJid, sender, text } });
    }

    async getBatchByGroup(onlyGroupJid?: string): Promise<Map<string, { sender: string; text: string }[]>> {
        ...
        const byGroup = new Map<string, { sender: string; text: string }[]>();
        for (const row of rows) {
            const arr = byGroup.get(row.groupJid) ?? [];
            arr.push({ sender: row.sender, text: row.text });
