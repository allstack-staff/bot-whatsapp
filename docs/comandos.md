...
**Comportamento:** violação objetiva (discriminação, conteúdo explícito, ato ilícito, etc. — veja [Regras](regras.html)) → banimento de comunidade direto, **a(s) mensagem(ns) que causou(aram) a violação é(são) apagada(s)** do grupo, com um aviso resumido público **no próprio grupo** (usuário, tipo, motivo) e o registro completo, revertível, no grupo de admins (veja abaixo). Violação que depende de julgamento de contexto (hoje só "divulgação de baixa participação" — a mais sujeita a interpretar errado se o conteúdo é relevante ao grupo ou não) **não bane sozinha**: propõe no grupo de admins e espera confirmação (✅ bane de verdade, ❌ dispensa). Qualquer advertência → mesmo mecanismo do `$asb advertir`, mesmo limite de 3/mês, mesmo escalonamento por reincidência, só avisada no grupo de admins, sem aviso público no grupo — **exceto publicação fora de contexto** (regra 8), que além da advertência já **remove a publicação** na hora (sem precisar de confirmação, só apaga a mensagem).
